...
operator()(const NodeID, const EdgeID via_eid, Intersection intersection) const
{
    ...
    if (detail::isMotorwayClass(in_data.road_classification.road_class))
    {
        ...
        std::for_each(intersection.begin(), intersection.end(), [](ConnectedRoad &road) {
            if (road.turn.instruction.type == TurnType::Ramp)
                road.turn.instruction.type = TurnType::ExitRamp;
        });
        return intersection;
    }
    ...
    else if (intersection.size() == 4)
    {
        ...
        for (auto &road : intersection)
        {
            ...
            else
            {
                ...
                road.turn.instruction = {TurnType::ExitRamp, getTurnDirection(road.turn.angle)};
            }
        }
    }
    else
